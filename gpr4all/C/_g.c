...
static char avg_var_docstring[] =
    "calcualate a monte carlo estimate of the average variance and its gradient ";

static PyObject *_g(PyObject *self, PyObject *args);

static PyObject *_avg_var(PyObject *self, PyObject *args);

static PyMethodDef module_methods[] = {
	    {"g"       , _g        , METH_VARARGS, g_docstring    },	  
	    {"avg_var" , _avg_var  , METH_VARARGS, avg_var_docstring    },
	    {NULL, NULL, 0, NULL}
};
...
PyMODINIT_FUNC init_avg_var(void) {

    PyObject *m = Py_InitModule3("_avg_var", module_methods, module_docstring);
    if (m == NULL)
        return;
...
static PyObject *_avg_var(PyObject *self, PyObject *args)
{
	...
	double r, d, reg;
	PyObject *xn_obj, *U_obj, *S_obj , *V_obj , *X_obj, *sample_obj;
    ...
    int nsteps = (int)PyArray_DIM(sample_array , 0); //  == nsteps
    int smpdim = (int)PyArray_DIM(sample_array , 1);
    ...
    if ( xnlen != veclen ) {
        	PyErr_SetString(PyExc_RuntimeError,
        							"Dimension mismatch:  len(xn) != specs.Xarr.shape[1]");
    		return NULL;
        }
    ...
    double *sample = (double*)PyArray_DATA(sample_array);
    double *xn     = (double*)PyArray_DATA(xn_array);


    /* Call the external C function to compute the covariance. */
    struct valGrad values = avgVar(U, S, V, X, sample, xn, r, d, reg,  nvecs, veclen,  nsteps);
    ...
    /* Build the output tuple */
	PyObject *ret = Py_BuildValue("dO", value, pyGrad );
	Py_DECREF(pyGrad);
...
static PyObject *_g(PyObject *self, PyObject *args)
{
  ...
  /* Call the external C function to compute the covariance. */
  struct valGrad values = g(U, S, V, X, x, xn, r, d, reg, nvecs, veclen);
