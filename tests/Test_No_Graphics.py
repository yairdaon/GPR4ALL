...
class Test(unittest.TestCase):
    ...
    def testUniform(self):
        ...
        self.assertTrue(    np.allclose( np.array([0]) , specs.kriging(np.array([20.0])) )      )
    ...
    def testGradientKrig(self):
        ...
        analytic  =    specs.kriging( x, True)[2]
    ...
    def testGradientSigSqr(self):
        ...
        x         =    np.array( [ 0.47])
        analytic  =    specs.kriging(x, True)[3]
        numeric   = ( specs.kriging(x+dx,  True)[1] - specs.kriging(x-dx, True)[1] )/(2*dx)
        ...
        numeric[0]  = (  specs.kriging(x + np.array([dx,0]), True)[1] - specs.kriging(x + np.array([-dx,0]), True)[1]  )/(2*dx)  
        numeric[1]  = (  specs.kriging(x + np.array([0,dx]), True)[1] - specs.kriging(x + np.array([0,-dx]), True)[1]  )/(2*dx)  
        self.assertTrue(np.allclose( analytic, numeric) )         
